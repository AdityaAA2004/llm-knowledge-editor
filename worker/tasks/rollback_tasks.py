...
def run_rollback(job_id: str, checkpoint_id: str) -> None:
    ...
    try:
        ...
        with stage("locate_checkpoint", "Locate checkpoint", "🔎"):
            with get_db_session() as db:
        ...
        with stage("load_weights", "Load checkpoint weights", "📦"):
        ...
        with stage("attach_scrubbers", "Re-attach scrubbers", "🔗"):
        ...
        with stage("swap_model", "Swap active model", "🔀"):
        ...
        with stage("reconcile_db", "Reconcile triple state", "🧮"):
            with get_db_session() as db:
