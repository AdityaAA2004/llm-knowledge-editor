...
#    instead of a (hidden_states, ...) tuple, so edit_output_fn and full_repr now handle
#    both shapes (previously `cur_out[0][...]` sliced the batch dim -> IndexError).
#  - delta is fp32 while the model runs fp16, so cast delta to the activation dtype
#    before the in-place add / final target (mirrors the compute_v patch).


def _hidden_states(out):
    """Return the hidden-states tensor from a decoder-layer output (tuple or tensor)."""
...
def compute_z(
    ...
    hidden_size = getattr(model.config, "n_embd", None) or model.config.hidden_size
    ...
    def edit_output_fn(cur_out, cur_layer):
        ...
        if cur_layer == hparams.layer_module_tmp.format(layer):
            hs = _hidden_states(cur_out)

            # Store initial value of the vector of interest
            if target_init is None:
                print("Recording initial value of v*")
                # Initial value is recorded for the clean sentence
                target_init = hs[0, lookup_idxs[0]].detach().clone()

            # Add intervened delta
            for i, idx in enumerate(lookup_idxs):
                hs[i, idx, :] += delta.to(hs.dtype)
