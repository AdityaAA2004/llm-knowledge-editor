...
- **All weight edits run as MEMIT, including single triples** (a MEMIT batch of one). ROME applies its full rank-one update at every layer in `hparams.layers` with no residual spreading — the original 5-layer config over-edited and collapsed the model into repeated-token output (diagnosed 2026-07-12). `POST /jobs/edit` coerces `edit_rome` → `edit_memit` (and `/jobs/{id}/rerun` coerces historical ROME jobs); the incident service and frontend send `edit_memit` directly. `run_rome_edit` and its hparams (now single-layer `[5]`) remain only for experimentation; nothing dispatches them.
